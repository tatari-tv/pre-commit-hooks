...
def check_function_call(filename, forbidden_function: list[str], suggest_replace: list[str]):
    check_accumulator = []
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            if isinstance(node.func, ast.Attribute):
                if node.func.attr in forbidden_function:
                    index = forbidden_function.index(node.func.attr)
...
def validate_args(args):
    if len(args.replacement_function_calls) != len(args.forbidden_function_calls):
...
def main(args):
    ...
    for file_path in args.file_list:
        return_code |= check_function_call(file_path, args.forbidden_function_calls, args.replacement_function_calls)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--forbidden-function-calls', nargs='+', help='Names of the function calls to check', required=True)
