...
from python_hooks.forbidden_function_calls import check_function_call
from python_hooks.forbidden_function_calls import main
from python_hooks.forbidden_function_calls import MESSAGE
from python_hooks.forbidden_function_calls import validate_args
from python_hooks.utills.ignore_check import FunctionCall

FILE = f'{dirname(__file__)}/data/forbidden_function_calls_sample.py'


@patch('builtins.print')
@patch('python_hooks.forbidden_function_calls.ignore_check')
...
def test_call_main_caught():
    args = argparse.Namespace(forbidden_function_calls=['split'], replacement_function_calls=['splitlines'], file_list=[FILE])
    actual = main(args)
    expected = 1
...
def test_call_main_pass():
    args = argparse.Namespace(forbidden_function_calls=['my_function'], replacement_function_calls=['my_other_function'], file_list=[FILE])
    actual = main(args)
    expected = 0
    assert actual == expected
...
def test_validate_args_caught():
    args = argparse.Namespace(
        forbidden_function_calls=['split', 'splat'], replacement_function_calls=['splitlines'], file_list=[FILE, FILE]
    )
